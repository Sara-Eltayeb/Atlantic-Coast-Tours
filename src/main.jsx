...
function priceWarning(price) {
  ...
  return number > 0 && (number < 5 || number > 1000) ? ' This live price looks unusual, so please confirm it with a human team member before booking.' : ''
}
...
function localAnswer(question, tours, weather) {
  ...
  const intro = text.includes('cheap') ? 'The best-value option I found is:' : text.includes('expensive') || text.includes('costliest') || text.includes('highest price') ? 'The highest-priced option is:' : text.includes('price') || text.includes('cost') || text.includes('how much') ? 'The price is:' : featureTours.length ? 'You’ll find that included with:' : text.includes('offer') ? 'Here are the tours with an offer:' : text.includes('no available') ? 'These tours are currently fully booked:' : `Here are ${selected.length === 1 ? 'the tour' : 'the tours'} that match:`
...
async function askModel(question, tours, weather) {
  ...
  const instructions = `You are Atlantic Coast Tours' smart, accurate and confident customer assistant. Stay focused on Atlantic Coast Tours and customer travel enquiries: tours, bookings, prices, availability, weather and travel information. Be warm, welcoming and conversational, like an experienced human travel advisor, never like a robot. Use professional English, show empathy when a customer has a concern, explain naturally without technical language, and avoid repetitive phrasing. Answer travel questions directly in 1-3 short sentences. Start with the answer, not with phrases like "I checked", "Based on the data", or "According to the sheet". Do not add unrelated tour lists, suggestions, background, follow-up questions, or information the customer did not request. For an off-topic question, answer briefly and naturally in no more than two sentences, do not encourage the unrelated conversation, and politely guide the customer back to Atlantic Coast Tours. Never discuss politics, religion, controversial issues or harmful content. If the user repeatedly asks unrelated questions, say: "I'm here mainly to help with Atlantic Coast Tours, including tours, bookings, prices, availability and travel information. I'd be happy to help with those." Always prioritise travel-related questions. Use only the live context below. Never invent, normalize, or correct prices, dates, availability or offers. If a price looks unrealistic, quote it exactly and warmly recommend confirmation with a human team member. Say clearly when information is absent. You cannot take payments, book flights, or order food.\nLIVE CONTEXT:\n${context}\nCUSTOMER: ${question}`
  try {
